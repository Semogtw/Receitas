...
declare
  ...
  target_is_admin_removed boolean := false;
begin
  perform private.assert_account_admin_actor(p_pair_id, p_actor_user_id);
  ...
  if not target_is_active then
    ...
    ) into target_is_admin_removed;
  end if;

  if not target_is_active and not target_is_admin_removed then
